...
! This heavily relieves the burden of tracking content and imaginary double ID's for the 
! NetCDF interface.
! 
  ...
  subroutine ncdf_inq_var(ncdf,name,exist,id,size,atts)
    ...
    if ( present(size) ) then
       ...
       do i = 1 , min(nids,ubound(size,1))
          ...
          call ncdf_get_dim(ncdf,trim(dim),size(i))
       end do
    end if
...
! Delete attributes
  ...
  subroutine ncdf_get_dim(ncdf,name,len)
    ...
    call ncdf_inq_dim(ncdf,name,len=len)
